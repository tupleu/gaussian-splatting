...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, pipe, renderArgs, train_test_exp, scene2, use_bg):
    ...
    if iteration in testing_iterations:
        ...
        validation_configs = ({'name': 'test', 'cameras' : [scene.getTestCameras()[idx % len(scene.getTestCameras())] for idx in range(0, 0, 1)]}, 
                              {'name': 'train', 'og' :[scene2.getTrainCameras()[idx % len(scene2.getTrainCameras())] for idx in range(0, 2, 1)],
                                    'cameras' : [scene.getTrainCameras()[idx % len(scene.getTrainCameras())] for idx in range(0, 2, 1)]})
